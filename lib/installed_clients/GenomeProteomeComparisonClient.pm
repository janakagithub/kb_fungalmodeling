package installed_clients::GenomeProteomeComparisonClient;
...
installed_clients::GenomeProteomeComparisonClient
...
sub new
{
    ...
    my $self = {
	client => installed_clients::GenomeProteomeComparisonClient::RpcClient->new,
	url => $url,
	headers => [],
    };
    my %arg_hash = @args;
...
# Authentication: ${method.authentication}
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "GenomeProteomeComparison._check_job",
        params => \@args});
    if ($result) {
        if ($result->is_error) {
...
{
    my($self, @args) = @_;
    my $job_id = $self->_blast_proteomes_submit(@args);
...
sub _blast_proteomes_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _blast_proteomes_submit (received $n, expecting 1)");
    }
    {
        my($input) = @args;
        my @_bad_arguments;
        ...
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _blast_proteomes_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_blast_proteomes_submit');
        }
    }
    my $context = undef;
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "GenomeProteomeComparison._blast_proteomes_submit",
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _blast_proteomes_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_blast_proteomes_submit');
    }
}
...
sub status
{
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "GenomeProteomeComparison._status_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for installed_clients::GenomeProteomeComparisonClient\n";
    }
    if ($sMajor == 0) {
        warn "installed_clients::GenomeProteomeComparisonClient version is $svr_version. API subject to change.\n";
    }
}
...
package installed_clients::GenomeProteomeComparisonClient::RpcClient;
