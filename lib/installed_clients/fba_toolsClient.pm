package installed_clients::fba_toolsClient;
...
installed_clients::fba_toolsClient
...
sub new
{
    ...
    my $self = {
	client => installed_clients::fba_toolsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
    my %arg_hash = @args;
...
{
    my($self, @args) = @_;
    my $job_id = $self->_predict_metabolite_biosynthesis_pathway_submit(@args);
    my $async_job_check_time = $self->{async_job_check_time};
    while (1) {
        Time::HiRes::sleep($async_job_check_time);
        $async_job_check_time *= $self->{async_job_check_time_scale_percent} / 100.0;
...
sub _predict_metabolite_biosynthesis_pathway_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _predict_metabolite_biosynthesis_pathway_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _predict_metabolite_biosynthesis_pathway_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_predict_metabolite_biosynthesis_pathway_submit');
        }
    }
    my $context = undef;
    if ($self->{service_version}) {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "fba_tools._predict_metabolite_biosynthesis_pathway_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _predict_metabolite_biosynthesis_pathway_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_predict_metabolite_biosynthesis_pathway_submit');
    }
}
...
{
    my($self, @args) = @_;
    my $job_id = $self->_build_metagenome_metabolic_model_submit(@args);
    my $async_job_check_time = $self->{async_job_check_time};
    while (1) {
        Time::HiRes::sleep($async_job_check_time);
        $async_job_check_time *= $self->{async_job_check_time_scale_percent} / 100.0;
...
sub _build_metagenome_metabolic_model_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _build_metagenome_metabolic_model_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _build_metagenome_metabolic_model_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_build_metagenome_metabolic_model_submit');
        }
    }
    my $context = undef;
    if ($self->{service_version}) {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "fba_tools._build_metagenome_metabolic_model_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _build_metagenome_metabolic_model_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_build_metagenome_metabolic_model_submit');
    }
}
...
{
    my($self, @args) = @_;
    my $job_id = $self->_fit_exometabolite_data_submit(@args);
    my $async_job_check_time = $self->{async_job_check_time};
    while (1) {
        Time::HiRes::sleep($async_job_check_time);
        $async_job_check_time *= $self->{async_job_check_time_scale_percent} / 100.0;
...
sub _fit_exometabolite_data_submit {
    my($self, @args) = @_;
# Authentication: required
    if ((my $n = @args) != 1) {
        Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
                                   "Invalid argument count for function _fit_exometabolite_data_submit (received $n, expecting 1)");
    }
    {
        my($params) = @args;
        my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
            my $msg = "Invalid arguments passed to _fit_exometabolite_data_submit:\n" . join("", map { "\t$_\n" } @_bad_arguments);
            Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
                                   method_name => '_fit_exometabolite_data_submit');
        }
    }
    my $context = undef;
    if ($self->{service_version}) {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "fba_tools._fit_exometabolite_data_submit",
        params => \@args, context => $context});
    if ($result) {
        if ($result->is_error) {
            Bio::KBase::Exceptions::JSONRPC->throw(error => $result->error_message,
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method _fit_exometabolite_data_submit",
                        status_line => $self->{client}->status_line,
                        method_name => '_fit_exometabolite_data_submit');
    }
}
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for installed_clients::fba_toolsClient\n";
    }
    if ($sMajor == 0) {
        warn "installed_clients::fba_toolsClient version is $svr_version. API subject to change.\n";
    }
}
...
package installed_clients::fba_toolsClient::RpcClient;
